    ...
    , ui(new Ui::chatwindow)
{
    ...
    QString ip = "192.168.43.188"; // 使用ens33的IP
    quint16 port = 8888;
...
// 初始化联系人列表
void chatwindow::initializeContactList(const QString &id) {
    QString contactRequest = QString("CONTACTLIST#%1").arg(id);
    emit sendMsgSignal(contactRequest.toUtf8());  // 通过信号发送给工作线程
...
//         format.setSampleSize(16);  // 16位样本
//         format.setCodec("audio/pcm");
//         format.setByteOrder(QAudioFormat::LittleEndian);
//         format.setSampleType(QAudioFormat::SignedInt);  // 有符号整数

//     // 创建音频输入源对象
//     audioSource = new QAudioInput(format, this);
